...
import { Location01Icon, Clock01Icon, ArrowLeft01Icon, Calendar01Icon, UserGroupIcon } from '@hugeicons/core-free-icons';
...
export const StoryboardViewer: React.FC<StoryboardViewerProps> = ({ data, originalMemories, tripDetails, onReset }) => {
  ...
  return (
    <div className="min-h-screen bg-stone-50 animate-fade-in relative dark:bg-slate-900">
      {/* Floating Header */}
      <nav className="fixed top-0 left-0 right-0 z-50 px-6 py-4 flex justify-between items-center pointer-events-none">
        <button
          onClick={onReset}
          className="pointer-events-auto flex items-center gap-2 bg-white/80 backdrop-blur-md text-stone-800 px-4 py-2 rounded-full shadow-sm hover:bg-white transition-all text-sm font-medium border border-stone-200/50 dark:bg-slate-800/80 dark:text-slate-100 dark:border-slate-600/50 dark:hover:bg-slate-700/80"
        >
        ...
        <div className="max-w-4xl mx-auto text-center relative z-10">
          ...
          {data.weatherImageUrl && (
            <div className="flex justify-center mb-10 animate-slide-up">
              <div className="relative group rounded-3xl overflow-hidden shadow-2xl border-4 border-white max-w-sm w-full aspect-square bg-stone-100 rotate-2 hover:rotate-0 transition-transform duration-500 dark:bg-slate-800 dark:border-slate-600">
                <img
          ...
          {!data.weatherImageUrl && (
            <div className="flex justify-center gap-2 mb-8 animate-slide-up">
              <div
                className="flex items-center gap-2 px-4 py-1.5 text-xs font-bold tracking-widest uppercase rounded-full bg-white border shadow-sm dark:bg-slate-800 dark:border-slate-600"
              ...
              {tripDetails.companions && (
                <div
                  className="flex items-center gap-2 px-4 py-1.5 text-xs font-bold tracking-widest uppercase rounded-full bg-white border shadow-sm text-slate-500 dark:bg-slate-800 dark:border-slate-600 dark:text-slate-400"
                >
          ...
          <h1 className="text-5xl md:text-7xl font-serif font-medium text-stone-900 mb-6 leading-tight animate-slide-up dark:text-slate-100" style={{ animationDelay: '100ms' }}>
            {data.title}
          </h1>

          <div className="flex items-center justify-center gap-2 text-stone-500 font-medium mb-8 animate-slide-up dark:text-slate-400" style={{ animationDelay: '200ms' }}>
            <HugeiconsIcon icon={Location01Icon} size={16} className="text-stone-400 dark:text-slate-500" />
      ...
      <div className="max-w-4xl mx-auto px-4 pb-32">
        <div className="relative">
          {/* Vertical Line */}
          <div className="absolute left-8 md:left-1/2 top-0 bottom-0 w-px bg-stone-200 transform md:-translate-x-1/2 dark:bg-slate-700" />
              ...
              >
                ...
                <div className="flex-1 pl-16 md:pl-0">
                  <div className="relative group overflow-hidden rounded-2xl shadow-lg transition-transform duration-700 hover:scale-[1.01]">
                    <div className="aspect-[4/3] bg-stone-200 dark:bg-slate-700">
                      <img
                ...
                <div className="flex-1 pl-16 md:pl-0 flex flex-col justify-center">
                  <div className={`space-y-4 ${isEven ? 'md:text-right md:pr-12' : 'md:text-left md:pl-12'}`}>
                    ...
                    <h2 className="text-2xl font-serif text-stone-800 dark:text-slate-100">
                      {segment.caption}
                    </h2>

                    <p className="text-stone-600 leading-relaxed dark:text-slate-300">
                      {segment.narrative}
                    </p>

                    <div className={`hidden md:flex flex-wrap gap-2 pt-2 ${isEven ? 'justify-end' : 'justify-start'}`}>
                      {segment.tags.map(tag => (
                        <span key={tag} className="text-[10px] px-2 py-1 bg-white border border-stone-200 rounded-full text-stone-500 uppercase tracking-wide dark:bg-slate-700 dark:border-slate-600 dark:text-slate-300">
                          {tag}
                        </span>
                      ))}
                    </div>
                    ...
                    {segment.estimatedTimeOfDay && (
                      <div className={`flex items-center gap-1.5 text-xs text-stone-400 ${isEven ? 'md:justify-end' : 'md:justify-start'} dark:text-slate-500`}>
          ...
          {/* End Node */}
          <div className="absolute left-8 md:left-1/2 bottom-0 w-3 h-3 bg-stone-300 rounded-full transform -translate-x-1/2 translate-y-1/2 dark:bg-slate-600" />
        </div>

        <div className="text-center mt-20">
          <p className="font-serif italic text-stone-400 text-lg dark:text-slate-500">Fin.</p>
        </div>
      </div>
    </div>
  );
